...
class AxisPreview(vcswidget.QVCSWidget):
	def __init__(self, vcsaxis, parent=None):
		super(AxisPreview, self).__init__(parent=parent)
		self.axis = vcsaxis
	...
	def update(self):
		...
		template.drawTicks(self.axis.var, self.axis.gm, self.canvas, axis_orientation, axis_number, (template.data.x1, template.data.x2,template.data.y1, template.data.y2), wc)
		self.canvas.backend.renWin.Render()

	def resizeEvent(self, ev):
		super(AxisPreview, self).resizeEvent(ev)
		self.update()
